...
class QueryRequest(BaseModel):
    ...
    top_k: Optional[int] = Field(
        None, description="Number of results to retrieve (overrides default)"
    )
    include_sources: bool = Field(
        True, description="Whether to include source passages in the response"
    )
...
class BatchQueryRequest(BaseModel):
    ...
    include_sources: bool = Field(
        True, description="Whether to include source passages"
    )
...
class HealthResponse(BaseModel):
    ...
    status: str = Field(
        ..., description="Status of the service (e.g., 'ok', 'unhealthy')"
    )
