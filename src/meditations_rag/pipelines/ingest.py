...
from meditations_rag.services.metadata import ChunkMetadata, MetadataExtractorService
...
class IngestPipeline:
    ...
    def _enrich_chunk_metadata(self, chunk, extracted_metadata: ChunkMetadata) -> None:
        ...
        chunk.metadata["topic"] = (
            extracted_metadata.topic.value
        )  # Store enum value as string
        chunk.metadata["entities"] = extracted_metadata.entities
        chunk.metadata["philosophical_concepts"] = (
            extracted_metadata.philosophical_concepts
        )
    ...
    async def ingest(self, file_path: str, upsert_to_vector_store: bool = True):
        ...
        if self.metadata_extractor and settings.rag.metadata_extraction_enabled:
            ...
            chunk_metadata_list = await self.metadata_extractor.extract_batch_metadata(
                chunk_texts
            )
            ...
            for chunk, extracted_metadata in zip(chunks, chunk_metadata_list):
                if extracted_metadata is not None:
                    ...
                    logger.warning(
                        f"Chunk {chunk.node_id} metadata extraction failed, keeping original metadata"
                    )

            logger.info(
                f"Enriched {enriched_count}/{len(chunks)} chunks with extracted metadata"
            )
        ...
        if upsert_to_vector_store and self.vector_store and self.embedding_service:
            ...
            chunk_results = await self.vector_store.upsert_chunks_batch(
            ...
            question_vectors_by_chunk = (
                await self.embedding_service.embed_questions_by_chunk(chunks)
            )
        ...
        logger.info(
            f"Completed ingestion for file: {file_path} in {end_time - start_time:.2f} seconds"
        )
