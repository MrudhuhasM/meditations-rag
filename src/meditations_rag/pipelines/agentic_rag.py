import operator
import time
from typing import List, Dict, Any, TypedDict, Optional, Annotated, Union, Literal
...
class AgentState(TypedDict):
    ...
    query: str
    messages: List[Dict[str, str]]  # Simple message history: {"role": "user"|"ai", "content": "..."}
...
class EvaluationResult(BaseModel):
    ...
    grounded: bool = Field(description="Is the answer fully supported by the retrieved context?")
    relevant: bool = Field(description="Does the answer directly address the user's question?")
    feedback: Optional[str] = Field(description="If not grounded or relevant, provide specific feedback on what is missing or wrong.")
...
async def controller_node(state: AgentState, llm: LLMBase) -> Dict[str, Any]:
    ...
    if docs:
        ...
        context_summary += "Content snippets: " + " | ".join([d.text[:100] + "..." for d in docs[:3]])
    else:
...
Decide the next step."""
    ...
    try:
        decision: ControllerDecision = await llm.generate_structured(prompt, ControllerDecision)
        ...
        if decision.search_query:
            updates["search_queries"] = state.get("search_queries", []) + [decision.search_query]
...
async def retriever_node(state: AgentState, retrieval_tool: RetrievalTool) -> Dict[str, Any]:
    """Executes the retrieval."""
    query = state.get("current_search_query", state["query"])
...
If 'No', provide feedback on what is missing or hallucinated."""
    ...
    try:
        result: EvaluationResult = await llm.generate_structured(prompt, EvaluationResult)
