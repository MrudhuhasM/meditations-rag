...
from meditations_rag.services.retrieval import (
    QueryRewriter,
    RetrievalResult,
    RetrievalService,
)
...
class RAGResponse(BaseModel):
    ...
    total_time_seconds: float = Field(
        default=0.0, description="Total pipeline execution time"
    )
...
class RAGPipeline:
    ...
    def __init__(
        ...
        Args:
            ...
            enable_guardrails: Enable LLM-based safety and relevance checks
        """
        self.llm = llm
        ...
        if enable_guardrails:
            self.guardrail_service = GuardrailService(llm=self.llm)
            ...
            if include_metadata:
                ...
                if result.matched_question:
                    metadata_lines.append(
                        f"Related Question: {result.matched_question}"
                    )
...
**Your Answer:**"""
    ...
    async def _generate_answer(
        ...
        try:
            ...
            metadata = {"generation_time_seconds": generation_time, "model": model_name}
            ...
            logger.error(
                f"Unexpected error during answer generation: {e}", exc_info=True
            )
    ...
    async def query(
        ...
        try:
            ...
            if self.guardrail_service:
                logger.debug("Step 0: Checking guardrails")
                guardrail_result = await self.guardrail_service.validate(query)

                if not guardrail_result.allowed:
                    logger.info(
                        f"Query blocked by guardrail: {guardrail_result.reason}"
                    )
                    return RAGResponse(
                        query=query,
                        answer=guardrail_result.refusal_message
                        or "I cannot answer this query.",
                        sources=[],
                        retrieval_metadata={
                            "blocked": True,
            ...
            retrieval_metadata = {
                ...
                "avg_score": sum(r.score for r in retrieval_results)
            ...
            raise MeditationsRAGException(
                f"RAG pipeline failed: {e}", details={"query": query, "error": str(e)}
            ) from e
    ...
    async def batch_query(
        ...
        responses = await asyncio.gather(
            *[process_with_semaphore(q) for q in queries], return_exceptions=True
        )
        ...
        for i, response in enumerate(responses):
            if isinstance(response, Exception):
                ...
                final_responses.append(cast(RAGResponse, response))
    ...
    async def stream_query_events(
        ...
        if self.guardrail_service:
            try:
                ...
                logger.error(f"Guardrail check failed in stream: {e}")
                # Continue if guardrail fails (fail open) or handle error
    ...
    async def stream_answer(self, query: str, top_k: Optional[int] = None):
        """
...
def create_rag_pipeline(
    llm: LLMBase, embedding_provider: EmbeddingBase, **kwargs
) -> RAGPipeline:
    """
