...
class QueryMetadata(BaseModel):
    ...
    topic: Optional[MeditationsTopic] = Field(
        default=None, description="Primary topic if identifiable"
    )
    entities: List[str] = Field(
        default_factory=list, description="Named entities mentioned"
    )
    philosophical_concepts: List[str] = Field(
        default_factory=list, description="Stoic/philosophical concepts"
    )
    stoic_practices: List[str] = Field(
        default_factory=list, description="Stoic practices mentioned"
    )
...
Provide high-quality metadata to maximize retrieval precision."""
        ...
        try:
            result = await self.llm.generate_structured(
                prompt=prompt, response_model=QueryMetadata
            )
...
class RetrievalResult(BaseModel):
    ...
    chunk_score: Optional[float] = Field(
        default=None, description="Score from chunk collection"
    )
    question_score: Optional[float] = Field(
        default=None, description="Score from question collection"
    )
    text: str = Field(description="Retrieved chunk text")
    metadata: Dict[str, Any] = Field(default_factory=dict, description="Chunk metadata")
    matched_question: Optional[str] = Field(
        default=None, description="Question that matched (if from question retrieval)"
    )
...
class RetrievalService:
    ...
    async def _retrieve_from_chunks(self, query: str, top_k: int) -> List[ScoredPoint]:
        """
    ...
    async def _retrieve_with_metadata_filter(
        ...
        try:
            ...
            logger.debug(f"Retrieved {len(results)} results with {filter_field} filter")
            return results
    ...
    async def _retrieve_metadata_aware(
        ...
        if query_metadata.philosophical_concepts:
            retrieval_tasks.append(
                self._retrieve_with_metadata_filter(
                    query_vector,
                    "philosophical_concepts",
                    query_metadata.philosophical_concepts,
                    top_k_per_filter,
                )
            )
        ...
        if query_metadata.stoic_practices:
            retrieval_tasks.append(
                self._retrieve_with_metadata_filter(
                    query_vector,
                    "stoic_practices",
                    query_metadata.stoic_practices,
                    top_k_per_filter,
                )
            )
        ...
        for metadata_type, result in zip(metadata_types, results):
            if isinstance(result, Exception):
                logger.warning(
                    f"Metadata retrieval failed for {metadata_type}: {result}"
                )
                metadata_results[metadata_type] = []
            elif isinstance(result, list):
                metadata_results[metadata_type] = result
                logger.debug(f"Retrieved {len(result)} results for {metadata_type}")
            else:
                logger.warning(
                    f"Unexpected result type for {metadata_type}: {type(result)}"
                )
                metadata_results[metadata_type] = []
    ...
    def _fuse_scores_with_metadata(
        ...
        for point in question_results:
            ...
            if raw_chunk_id:
                chunk_id_str = str(raw_chunk_id)
                if (
                    chunk_id_str not in question_scores
                    or point.score > question_scores[chunk_id_str][0]
                ):
                    q_text = str(payload.get("question", ""))
                    question_scores[chunk_id_str] = (point.score, q_text, point)

        # Track metadata matches for boosting
        metadata_matches: Dict[
            str, Set[str]
        ] = {}  # chunk_id -> set of metadata_types matched
        ...
        for chunk_id in all_chunk_ids:
            ...
            matched_question_val: Optional[str] = cast(
                Optional[str],
                question_info[1] if len(question_info) > 1 else None,
            )
            ...
            final_score = (
                chunk_score_val + self.alpha * question_score_val + boost_score
            )
            ...
            result = RetrievalResult(
                ...
                text=text,
                metadata=metadata,
                matched_question=matched_question_val,
            )
        ...
        logger.info(
            ...
            f"Top score: {fused_results[0].score:.4f}"
            if fused_results
            else "No results"
        )
    ...
    def _fuse_scores(
        self, chunk_results: List[ScoredPoint], question_results: List[ScoredPoint]
    ) -> List[RetrievalResult]:
        """
    ...
    async def retrieve(
        ...
        metadata_enabled = (
            use_metadata if use_metadata is not None else self.enable_metadata_retrieval
        )
        ...
        if (
            ...
            and not (chunk_only or question_only)
        ):
        ...
        if question_only:
            ...
            logger.info(
                f"Running {len(retrieval_tasks)} parallel retrievals: {', '.join(task_names)}"
            )
            ...
            metadata_results = (
                cast(Dict[str, List[ScoredPoint]], results[2])
                if len(results) > 2
                else {}
            )
            ...
            if isinstance(metadata_results, Exception):
                ...
                logger.warning(
                    f"Unexpected metadata result type: {type(metadata_results)}"
                )
                metadata_results = {}
        ...
        if chunk_only or question_only:
            ...
            for point in source_results[:k]:
                ...
                chunk_id = (
                    str(point.id)
                    if chunk_only
                    else payload.get("chunk_id", str(point.id))
                )
            ...
            logger.info(
                f"Single-source retrieval complete: {len(results_list)} results"
            )
            return results_list
        else:
