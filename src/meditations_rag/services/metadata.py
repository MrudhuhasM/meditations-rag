...
class MetadataExtractorService:
    ...
    async def extract_batch_metadata(
        ...
        for batch_start in range(0, len(chunks), self.batch_size):
            ...
            logger.info(f"Processing batch {batch_start//self.batch_size + 1}: chunks {batch_start}-{batch_end-1}")
            ...
            logger.info(f"Batch completed: {success_count}/{len(batch_chunks)} successful extractions")
        
        # Calculate overall stats
        total_success = sum(1 for r in results if r is not None)
        success_rate = (total_success / len(chunks)) * 100 if chunks else 0
        
        logger.info(f"Metadata extraction completed: {total_success}/{len(chunks)} ({success_rate:.1f}%) successful")
