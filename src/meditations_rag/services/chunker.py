...
class ChunkerService:
    ...
    def chunk_documents(self, documents: List[Document]) -> List[BaseNode]:
        ...
        for i, batch in enumerate(batches):
            ...
            if (
                not chunked_docs and batch
            ):  # Only count as failure if batch was not empty but result is empty
                ...
                failure_count += 1
                logger.warning(
                    f"Batch {i + 1} failed to chunk or produced no chunks. Total failures: {failure_count}"
                )

            logger.info(
                f"Completed batch {i + 1}/{len(batches)}: Generated {len(chunked_docs)} chunks"
            )
        ...
        logger.info(
            f"Chunking completed: {len(chunks)} chunks created with {failure_count} failures."
        )
        return chunks
