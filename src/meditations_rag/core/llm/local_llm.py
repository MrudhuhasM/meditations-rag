...
class LocalLLM(BaseOpenAILLM):
    def __init__(self):
        ...
        client = AsyncOpenAI(
            api_key=local_settings.api_key.get_secret_value() if local_settings.api_key else None,
            base_url=local_settings.api_base_url,
        )
        super().__init__(client, local_settings, "local_llm")
    ...
    async def generate(self, prompt: str) -> str:
        ...
        try:
            return await self._generate_chat_completion(prompt, system_message=system_message)
