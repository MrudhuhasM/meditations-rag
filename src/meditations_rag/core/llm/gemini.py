...
class GeminiLLM(LLMBase):
    ...
    async def generate(self, prompt: str) -> str:
        try:
            ...
            if self.settings.reasoning_enabled and "2.5" in self.settings.llm_model_name:
                logger.debug(f"Enabling thinking mode for Gemini 2.5 model: {self.settings.llm_model_name}")
            ...
            logger.debug(f"Calling Gemini API with model: {self.settings.llm_model_name}")
            ...
            logger.debug(f"Successfully generated response from Gemini")
            ...
            if 'timeout' in error_message or 'deadline' in error_message:
                ...
                raise LLMTimeoutError(
                    f"Gemini API request timed out",
                    provider="gemini",
                    details={
                        "error": str(e),
                        "model": self.settings.llm_model_name
            ...
            logger.error(f"Unexpected error during Gemini generation: {str(e)}", exc_info=True)
            raise LLMAPIError(
    ...
    async def generate_structured(self, prompt: str, response_model: type[BaseModel]) -> BaseModel:
        try:
            ...
            # Add thinking config for Gemini 2.5 models when reasoning is enabled
            if self.settings.reasoning_enabled and "2.5" in self.settings.llm_model_name:
                logger.debug(f"Enabling thinking mode for Gemini 2.5 structured output")
                config_dict["thinking_config"] = types.ThinkingConfig(
            ...
            logger.debug(f"Calling Gemini API for structured output with model: {self.settings.llm_model_name}")
            response = await self.client.aio.models.generate_content(
                model=self.settings.llm_model_name,
                contents=prompt,
                config=config,
            ...
            try:
                ...
                logger.error(f"Failed to parse Gemini response into Pydantic model: {str(parse_error)}")
                raise LLMResponseError(
            ...
            logger.error(f"Unexpected error during Gemini structured generation: {str(e)}", exc_info=True)
            raise LLMAPIError(
