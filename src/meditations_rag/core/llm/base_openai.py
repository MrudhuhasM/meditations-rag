    ...
    async def _generate_chat_completion(
        ...
        try:
            ...
            if response.choices[0].message.content is None:
                logger.warning(
                    f"{self.provider_name} API returned None for message content"
                )
                raise LLMResponseError(
            ...
            logger.debug("Successfully generated response using Chat Completions API")
            return response.choices[0].message.content

        except Exception as e:
            self._handle_error(e)

    async def _generate_structured_common(
        self,
        prompt: str,
        ...
        try:
            ...
            if not response.choices or len(response.choices) == 0:
                logger.error(
                    f"{self.provider_name} API returned no choices for structured output"
                )
                raise LLMResponseError(
            ...
            if response.choices[0].message.refusal:
                logger.warning(
                    f"{self.provider_name} model refused structured output: {response.choices[0].message.refusal}"
                )
                raise LLMResponseError(
            ...
            if parsed is None:
                logger.error(
                    f"Failed to parse {self.provider_name} structured response"
                )
                raise LLMResponseError(
            ...
            logger.debug("Successfully generated structured output")
            return parsed

        except Exception as e:
            self._handle_error(e, response_model=response_model)

    def _handle_error(self, e: Exception, response_model=None) -> NoReturn:
