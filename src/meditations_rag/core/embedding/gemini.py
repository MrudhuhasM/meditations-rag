        ...
        try:
            ...
            response = await self.client.aio.models.embed_content(
                ...
                config=types.EmbedContentConfig(
                    output_dimensionality=settings.rag.embedding_dimension
                ),
            )
            ...
            if "timeout" in error_message or "deadline" in error_message:
                ...
                raise EmbeddingTimeoutError(
                    "Gemini embeddings API request timed out",
                    provider="gemini",
                    details={"error": str(e), "model": self.model_name},
                ) from e
            ...
            logger.error(
                f"Unexpected error during Gemini embedding: {str(e)}", exc_info=True
            )
            raise EmbeddingAPIError(
    ...
    async def _embed_batch(self, texts: list[str]) -> list[list[float]]:
        ...
        for i, embed in enumerate(response.embeddings):
            ...
            if len(embed) != settings.rag.embedding_dimension:
                logger.error(
                    f"Embedding {i} dimension mismatch: expected {settings.rag.embedding_dimension}, got {len(embed)}"
                )
                raise EmbeddingDimensionMismatchError(
