        ...
        try:
            ...
            if 'rate' in error_message or 'quota' in error_message or 'limit' in error_message:
                ...
                raise EmbeddingRateLimitError(
                    f"Gemini rate limit exceeded. Please wait before retrying.",
                    provider='gemini',
                    details={'error': str(e), 'model': self.model_name, 'max_retries': settings.gemini.max_retries, 'text_count': len(texts)}
                ) from e
            if 'timeout' in error_message or 'deadline' in error_message:
                logger.error(f"Gemini API timeout: {str(e)}")
                raise EmbeddingTimeoutError(
                    f"Gemini embeddings API request timed out",
                    provider='gemini',
                    details={'error': str(e), 'model': self.model_name, 'text_count': len(texts)}
                ) from e
            logger.error(f"Gemini client error: {str(e)}")
            raise EmbeddingAPIError(
                f"Gemini client error: {str(e)}",
            ...
            logger.error(f"Unexpected error during Gemini batch embedding: {str(e)}", exc_info=True)
            raise EmbeddingAPIError(
                f"Unexpected error during Gemini batch embedding generation: {str(e)}",
                provider='gemini',
                model=self.model_name,
                text_count=len(texts),
                error_type='unexpected_error',
